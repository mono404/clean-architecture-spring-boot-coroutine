...
class PostHandler(
    ...
    suspend fun create(serverRequest: ServerRequest): ServerResponse {
        ...
        val request = PostCreateOrUpdateRequest.fromPart(parts)
        val mediaFiles = parts["mediaFiles"]?.filterIsInstance<FilePart>() ?: emptyList()

        return ok(postCommandUseCase.create(memberId, request, mediaFiles))
    }
    ...
    suspend fun update(serverRequest: ServerRequest): ServerResponse {
        ...
        val request = PostCreateOrUpdateRequest.fromPart(parts)
        val mediaFiles = parts["mediaFiles"]?.filterIsInstance<FilePart>() ?: emptyList()
        return ok(postCommandUseCase.update(memberId, postId, request, mediaFiles))
    }

    suspend fun delete(serverRequest: ServerRequest): ServerResponse {
        val memberId = serverRequest.getMemberId()
        val postId = serverRequest.pathVariable("postId").toLong()
        postCommandUseCase.delete(memberId, postId)
        return noContent()
    }
