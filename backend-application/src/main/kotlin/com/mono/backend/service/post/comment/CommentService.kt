...
class CommentService(
    ...
    override suspend fun update(commentId: Long, request: CommentUpdateRequest): CommentResponse {
        ...
        eventDispatcherPort.dispatch(
            type = EventType.POST_COMMENT_UPDATED,
            payload = CommentUpdatedEventPayload.from(comment, count(comment.postId)),
        )
    ...
    private suspend fun delete(comment: Comment) {
        commentPersistencePort.deleteById(comment.commentId)
        postCommentCountPersistencePort.decrease(comment.postId)
    ...
    override suspend fun readAllChildren(
        postId: Long,
        commentId: Long
    ): List<CommentResponse> {
        val comments = commentPersistencePort.findAllChildren(postId, commentId)
        return comments.map(CommentResponse::from)
    }
