...
data class Post(
    ...
    companion object {
        ...
        private const val LOCAL_IMAGE_PATTERN = "(?<=!\\[]\\()([^\\s\\)]+)"
    ...
    fun containsImages(): Boolean {
        return content.contains(Regex(LOCAL_IMAGE_PATTERN))
    }

    fun extractLocalImageUrls(): List<String> {
        val appImages = Regex(LOCAL_IMAGE_PATTERN)
            .findAll(content)
            .map { it.value }
            .toList()
    ...
    fun replaceImageUrls(urlMap: Map<String, String>): Post {
        val updatedContent = urlMap.entries.fold(content) { acc, (localPath, s3Path) ->
            acc.replace(localPath, s3Path)
        }
