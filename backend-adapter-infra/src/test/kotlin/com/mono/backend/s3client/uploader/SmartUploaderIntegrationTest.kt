...
class SmartUploaderIntegrationTest {
    ...
    fun setUp() {
        ...
        val config = UploadConfig(
            multipartThreshold = 1024 * 1024, //1MB
            multipartPartSize = 512 * 1024,  // 512KB
        ...
        client = WebTestClient.bindToController(TestController(smartUploader)).build()
    }
...
//        val response = smartUploader.upload("upload/single_file.png", filePart, null)
    ...
    fun `upload large file as multipart`() = runTest {
        val content = ByteArray(2_000_000) { 0x2 } // 2MB
        val fakeFilePart = FakeFilePart("large.mp4", content)

//        val response = smartUploader.upload("samples/large.mp4", fakeFilePart, null)
        val response = smartUploader.uploadMultipart("upload/single_file.png", fakeFilePart).awaitSingle()
    ...
    fun `resume interrupted upload`() = runTest {
        ...
        // 1차 업로드 -> 중도 Exception
        runCatching { smartUploader.upload("sample/resume.mov", interrupted, null) }

        // 2차 정산 업로드
        val response = smartUploader.upload("sample/resume.mov", FakeFilePart("resume.mov", data), null)
        assertThat(response.eTag).isNotBlank()
    }
    ...
    fun `progress callback called`() = runTest {
        ...
        smartUploader.upload("sample/progress.bin", FakeFilePart("progress.bin", data), callback)
